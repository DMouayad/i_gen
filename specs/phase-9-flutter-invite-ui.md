...
9. As a customer opening this app, I want guidance toward the web app rather than invoicing screens, so that I do not work in the wrong place.
...
- Role gating mirrors the database matrix: admin sees everything; employee sees invoices fully but products, prices, and categories read-only (edit affordances hidden/disabled with explanation); customer credentials see guidance toward the web app. Gating is UI convenience only — the database remains the enforcer.
...
- The server function and database rules themselves (adjacent phases). Orders and customer-list screens (dedicated phase). Real-email linking UI. Password-strength meters beyond the platform default. Multi-device session management. Any sync-engine changes beyond consuming the already-cached owner id.
