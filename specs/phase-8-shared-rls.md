...
5. As a distributor opening this Flutter app, I want no access to invoices, products, or prices here, so that I am guided to the web app instead.
...
10. As support, I want a documented two-user check (admin row visible to employee, employee invoice visible to admin, distributor sees nothing), so that regressions are caught in minutes.
...
- Permission matrix enforced in the database, not just the UI: admin gets full read/write on all five; employee gets read/write on invoices and invoice lines and read-only on products, prices, and price categories; distributor gets no read and no write on any of the five from this app's credentials. Write of a forged creator id is rejected.
...
- Verification seam: a scripted role-matrix check against a preview project (admin seeds one row per table; employee reads all five, writes only invoices/lines, and is rejected writing prices; distributor reads zero) must pass before client work depending on sharing begins.
...
- Tested: employee sees admin-seeded rows; employee invoice write accepted; employee price write rejected; distributor reads zero and writes rejected; forged creator id rejected; signed-out local use unaffected.
...
- Decided: distributors are fully excluded from these five tables in this app; their data surface is orders plus a distributor-visible product read served by the web app's rules, defined in the orders phase.
