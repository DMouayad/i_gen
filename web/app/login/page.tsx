...
import { NoAccountError, NotDistributorError, useAuth } from "@/lib/auth";
...
function LoginForm() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      else if (e instanceof NotDistributorError) setError(t.notDistributor);
      else if (
        e instanceof Error &&
  ...
  return (
    <div className="card flex flex-col gap-3 max-w-md">
      ...
      <p className="muted text-sm">{t.signInCta}</p>
